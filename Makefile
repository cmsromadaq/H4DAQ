...
CXX		=g++
LD		=g++
CXXFLAGS	=-O2 -ggdb -std=gnu++0x -Wall -m64
...
$(addprefix $(BINDIR)/,$(Packages)): $(BINDIR)/% : $(BASEDIR)/test/%.$(SrcSuf) $(StatLib) | $(BINDIR)
	@echo $(call InfoLine , $@ )
	$(CXX) -o $@ $< $(CXXFLAGS)  $(StatLib) -I$(INC_DIR) -I$(HDIR) $(LDFLAGS) 
...
$(BINDIR)/%.$(ObjSuf): $(SRCDIR)/%.$(SrcSuf) $(HDIR)/%.$(HeadSuf)
	@echo $(call InfoLine , $@ )
	$(CXX) -o $@ $(SRCDIR)/$*.$(SrcSuf) $(CXXFLAGS)  -c  -I$(INC_DIR) -I$(HDIR) $(LDFLAGS)

#.d
$(BINDIR)/%.$(DepSuf): $(SRCDIR)/%.$(SrcSuf) $(HDIR)/%.$(HeadSuf)
	@echo $(call InfoLine , $@ )
	$(CXX) -o $@ $(SRCDIR)/$*.$(SrcSuf) $(CXXFLAGS) -M  -I$(INC_DIR) -I$(HDIR) $(LDFLAGS)
