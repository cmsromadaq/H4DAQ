...
for machine in pcethtb1 cms-h4-03 ; do 
	...
	mydataro="cd DAQ/H4DAQ; ./bin/datareadout -d -c data/config_${machine}_DR.xml -v ${verbosity} -l /tmp/log_h4daq_datareadout_\$(date +%s).log  > /tmp/log_h4daq_start_dr_${machine}_\$(date +%s).log" 
...
for machine in pcethtb2 ; do 
	...
	myrc="cd DAQ/H4DAQ; nohup ./bin/runcontrol  -d -c data/config_${machine}_RC.xml -v ${verbosity} -l /tmp/log_h4daq_runcontrol_\$(date +%s).log >  /tmp/log_h4daq_start_rc_${machine}_\$(date +%s).log " 
...
for machine in pcethtb2 ; do 
	...
	myeb="cd DAQ/H4DAQ; nohup ./bin/eventbuilder  -d -c data/config_${machine}_EB.xml -v ${verbosity} -l /tmp/log_h4daq_eventbuilder_\$(date +%s).log >  /tmp/log_h4daq_start_eb_${machine}_\$(date +%s).log " 
	[ "${dryrun}" == "0" ] || {  echo "$mycommand" ; echo "$mydatarc" ; continue; }
	[ "${start_eb}" == "0" ] && continue;
	## compile
	ssh ${machine}.cern.ch "${mycommand}" 2>&1 | tee /tmp/log_h4daq_update_$machine.log ;
