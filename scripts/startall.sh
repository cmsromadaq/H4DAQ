...
#	[ "${start_dr}" == "0" ] && continue;
	## compile
	[ "${norecompile}" == "1" ] || ssh ${daquser}@${machine} /bin/bash -l -c \'"${mycommand}"\' 2>&1 | tee /tmp/log_h4daq_update_$machine_${USER}.log | col1 | col2 
	## launch the daemon
	echo "-----------------------------"
	echo "START DATAREADOUT on $machine"
	echo "-----------------------------"
	ssh ${daquser}@${machine} /bin/bash -l -c \'"${mydataro}"\' 2>&1 | tee  /tmp/log_h4daq_start_dr_{machine}_$(date +%s)_${USER}.log ;
...
#	[ "${start_rc}" == "0" ] && continue;
	...
	echo "-----------------------------"
	ssh ${daquser}@${machine} /bin/bash -l -c \'"${myrc}"\' 2>&1 | tee /tmp/log_h4daq_start_rc_${machine}_$(date +%s)_${USER}.log ;
...
#	[ "${start_eb}" == "0" ] && continue;
	## compile
	[ "${ebrecompile}" == "0" ] || ssh ${daquser}@${machine} /bin/bash -l -c \'"${mycommand}"\' 2>&1 | tee /tmp/log_h4daq_update_$machine_${USER}.log | col1 | col2
	## launch the daemon
	echo "-----------------------------"
	echo "START EVENTBUILDER on $machine"
	echo "-----------------------------"
	ssh ${daquser}@${machine} /bin/bash -l -c \'"${myeb}"\' 2>&1 | tee /tmp/log_h4daq_start_rc_${machine}_$(date +%s)_${USER}.log ;
